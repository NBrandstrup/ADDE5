...
architecture arch of IIR_Biquad_en is
	...
	signal Coef_b0 : std_logic_vector(COEFF_WIDTH-1 downto 0); -- b0		~ +0.2225548
	signal Coef_b1 : std_logic_vector(COEFF_WIDTH-1 downto 0); -- b1		~ +0.4451095
	signal Coef_b2 : std_logic_vector(COEFF_WIDTH-1 downto 0); -- b2		~ +0.2225548	

	signal Coef_a1 : std_logic_vector(COEFF_WIDTH-1 downto 0); -- a1		~ -0.3372905 	
	signal Coef_a2 : std_logic_vector(COEFF_WIDTH-1 downto 0); -- a2		~ +0.3049199

	-- define each pre gain sample flip flop
	signal ZFF_X0, ZFF_X1, ZFF_X2, ZFF_Y1, ZFF_Y2 : std_logic_vector(INPUT_WIDTH-1 downto 0) := (others => '0');

	-- define each post gain 64 bit sample
	signal pgZFF_X0_quad, pgZFF_X1_quad, pgZFF_X2_quad, pgZFF_Y1_quad, pgZFF_Y2_quad : std_logic_vector((INPUT_WIDTH+COEFF_WIDTH)-1 downto 0) := (others => '0');

	-- define each post gain 32 but truncated sample
	signal pgZFF_X0, pgZFF_X1, pgZFF_X2, pgZFF_Y1, pgZFF_Y2 : std_logic_vector(INPUT_WIDTH-1 downto 0) := (others => '0');

	-- define output double reg
	signal Y_out_double : std_logic_vector(INPUT_WIDTH-1 downto 0) := (others => '0');
...
begin
	...
	begin
		if (n_reset = '0') then
			...
			if (sample_trig = '1') then
				ZFF_X0 <= X_in(INPUT_WIDTH-1) & X_in(INPUT_WIDTH-1 downto 1);
				ZFF_X1 <= ZFF_X0;
				ZFF_X2 <= ZFF_X1;
				ZFF_Y1 <= Y_out_double;
				ZFF_Y2 <= ZFF_Y1;
	...
	-- COUNTER FOR TIMING 
	q_next <= (others => '0') when q_reset = '1'
	          else                      -- resets the counter 
	          q_reg + 1 when q_add = '1'
	          else                      -- increment count if commanded
	          q_reg;
	...
	begin
		if rising_edge(clk) then
			if (trunc_prods = '1') then
				pgZFF_X0 <= pgZFF_X0_quad((INPUT_WIDTH+COEFF_WIDTH)-3 downto ((INPUT_WIDTH+COEFF_WIDTH)-3)-(INPUT_WIDTH-1));
				pgZFF_X2 <= pgZFF_X2_quad((INPUT_WIDTH+COEFF_WIDTH)-3 downto ((INPUT_WIDTH+COEFF_WIDTH)-3)-(INPUT_WIDTH-1));
				pgZFF_X1 <= pgZFF_X1_quad((INPUT_WIDTH+COEFF_WIDTH)-3 downto ((INPUT_WIDTH+COEFF_WIDTH)-3)-(INPUT_WIDTH-1));
				pgZFF_Y1 <= pgZFF_Y1_quad((INPUT_WIDTH+COEFF_WIDTH)-3 downto ((INPUT_WIDTH+COEFF_WIDTH)-3)-(INPUT_WIDTH-1));
				pgZFF_Y2 <= pgZFF_Y2_quad((INPUT_WIDTH+COEFF_WIDTH)-3 downto ((INPUT_WIDTH+COEFF_WIDTH)-3)-(INPUT_WIDTH-1));
			end if;
		end if;
	end process;
	...
	begin
		if rising_edge(clk) then
			if (trunc_out = '1') then
				Y_out <= Y_out_double(INPUT_WIDTH-1 downto ((INPUT_WIDTH-1)-(OUTPUT_WIDTH-1)));
			end if;
		end if;
	end process;
end arch;
